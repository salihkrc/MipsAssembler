...
void jtype_instruction(char *instruction, int immediate, FILE *Out) {
    ...
    strcat(allcommands, opcode);
    strcat(allcommands, immediateStr);
    //strcat(allcommands, rtBin);
    //strcat(allcommands, rdBin);
    //strcat(allcommands, immediate);
    //strcat(allcommands, func);


    // to HEX //
    char *bin = allcommands ;
    char *a = bin;
    int num = 0;
    do{
